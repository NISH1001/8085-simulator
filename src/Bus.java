 ...
 * other devices) can write to the bus, and multiple can listen.
 * Devices that write need to register with the bus and get
 * a writer id (bus selector) and use that for writing. To read,
 * devices just call the getPins() method.
 * The pins of the bus can be in any of three states : high,low,
 * and floating (tri-stated). In the floating state they don't
 * affect any register bits.
 */
public class Bus
{
